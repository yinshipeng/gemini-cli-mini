...
import { discoverMcpTools } from "../mcp/mcp-client.js";
import { loadMcpConfigurations } from "../mcp/mcp-config.js";
...
export async function integrateMcpTools(toolRegistry, mcpServerCommand) {
  try {
    ...
    await discoverMcpTools(mcpServers, mcpServerCommand, toolRegistry);

    // Log the discovered tools
    const allTools = toolRegistry.listTools();
    const mcpTools = allTools.filter((toolName) => {
      const tool = toolRegistry.getTool(toolName);
      // Check if tool has serverName property (indicating it's an MCP tool)
  ...
  return {
    ...
    async executeTool(name, params) {
      ...
      try {
        ...
        if (tool.serverName) {
          throw new Error(
            `MCP tool '${name}' from server '${tool.serverName}' failed: ${error.message}`
          );
        }
        throw error;
      }
